...
def export_pdf(case, investigations) -> bytes:
    ...
    pdf.add_page()
    w = pdf.epw  # effective page width — avoids w=0 cursor-drift crash

    # Title
    pdf.set_font("Helvetica", "B", 18)
    pdf.set_text_color(99, 102, 241)
    pdf.cell(w, 10, "OSINT Investigation Report", new_x="LMARGIN", new_y="NEXT")

    # Case header
    pdf.set_font("Helvetica", "B", 14)
    pdf.set_text_color(30, 30, 30)
    pdf.ln(4)
    pdf.cell(w, 8, _pdf_safe(f"Case: {case.title}"), new_x="LMARGIN", new_y="NEXT")

    pdf.set_font("Helvetica", "", 10)
    pdf.set_text_color(80, 80, 80)
    pdf.cell(w, 6, _pdf_safe(f"Status: {case.status.replace('_', ' ').title()}  |  Priority: {case.priority.title()}"), new_x="LMARGIN", new_y="NEXT")
    pdf.cell(w, 6, _pdf_safe(f"Created: {case.created_at.strftime('%Y-%m-%d %H:%M') if case.created_at else 'Unknown'}"), new_x="LMARGIN", new_y="NEXT")
    if case.description:
        pdf.set_x(pdf.l_margin)
        pdf.multi_cell(w, 6, _pdf_safe(f"Description: {case.description}"), new_x="LMARGIN", new_y="NEXT")
    pdf.cell(w, 6, _pdf_safe(f"Report generated: {datetime.datetime.utcnow().strftime('%Y-%m-%d %H:%M')} UTC"), new_x="LMARGIN", new_y="NEXT")
    ...
    pdf.set_text_color(30, 30, 30)
    pdf.cell(w, 8, f"Investigations ({len(investigations)})", new_x="LMARGIN", new_y="NEXT")
    ...
    for inv in investigations:
        ...
        pdf.set_x(pdf.l_margin)
        pdf.cell(w, 7, _pdf_safe(f"[{kind_label}]  {inv.query}"), new_x="LMARGIN", new_y="NEXT")
